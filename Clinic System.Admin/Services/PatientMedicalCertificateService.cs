...
public class PatientMedicalCertificateService
{
    ...
    public PatientMedicalCertificateService(
    ...
    public Task<(MedicalCertificateOptions? Item, string? Error)> GetOptionsAsync() =>
        GetSingleAsync<MedicalCertificateOptions>(
            "/api/dental/certificates/options",
            "No se pudieron cargar las opciones de certificados.");

    public Task<(List<PatientMedicalCertificateSummary> Items, string? Error)> ListAsync(int patientId) =>
        GetListAsync<PatientMedicalCertificateSummary>(
            $"/api/dental/certificates/patient/{patientId}",
            "No se pudieron cargar los certificados del paciente.");

    public Task<(PatientMedicalCertificateDetail? Item, string? Error)> GetAsync(int certificateId) =>
        GetSingleAsync<PatientMedicalCertificateDetail>(
            $"/api/dental/certificates/{certificateId}",
            "No se pudo cargar el certificado.");

    public Task<(PatientMedicalCertificateDetail? Item, string? Error)> CreateAsync(
        int patientId,
        SavePatientMedicalCertificateRequest request) =>
        SendAsync<PatientMedicalCertificateDetail>(
            HttpMethod.Post,
            $"/api/dental/certificates/patient/{patientId}",
            request,
            "No se pudo guardar el certificado.");

    public Task<(PatientMedicalCertificateDetail? Item, string? Error)> UpdateAsync(
        int certificateId,
        SavePatientMedicalCertificateRequest request) =>
        SendAsync<PatientMedicalCertificateDetail>(
            HttpMethod.Put,
    ...
    public Task<(bool Success, string? Error)> DeleteAsync(int certificateId) =>
        SendStatusAsync(
            HttpMethod.Delete,
            $"/api/dental/certificates/{certificateId}",
            null,
    ...
    private async Task<(T? Item, string? Error)> SendAsync<T>(
        HttpMethod method,
        string url,
        object? payload,
    ...
    private async Task<(bool Success, string? Error)> SendStatusAsync(
        HttpMethod method,
        string url,
        object? payload,
        string fallback)
